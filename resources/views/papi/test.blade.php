...
<!-- Form for PAPI Test -->
<form id="testForm" action="{{ route('papi.store') }}" method="POST">
    @csrf
    ...
    @for ($i = 1; $i <= 90; $i++)
        <div class="form-group">
            ...
            <label><input type="radio" name="jwb{{ $i }}" value="0">{{ $questions[ $i - 1][0]["label"] }}</label>
            <br>
            <label><input type="radio" name="jwb{{ $i }}" value="1">{{ $questions[ $i - 1][1]["label"] }}</label>
           <br>
